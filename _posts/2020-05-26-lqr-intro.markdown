...
we only cover the discounted setting in the rest part of this post.
...
Going from discrte-time to continuous-time require us to formulate the following differently:
```
...
$$ J(u) = \int_0^T \gamma^{t} \mathbb{E}[c_t] dt = \int_0^T \gamma^{t} \mathbb{E}[x(t)^\top S x(t) + u(t)^\top R u(t)] dt $$
...
For `controllability`, it's almost the same as in the discrete-time except that the $$A, B$$ should be replaced by the discretized-version:
$$\tilde{A} = I + hA$$, $$\tilde{B} = hB$$.
...
The greedy action equation $$u_g = -H_{22}^{-1} H_{21}$$ involves an inverse of an estimate, which is risky.
