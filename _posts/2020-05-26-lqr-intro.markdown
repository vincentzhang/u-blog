...
$$ J(u_0, u_1, ...) = \sum_{k=0}^{N} \gamma^{k} \mathbb{E}[c_k] = \sum_{k=0}^{N-1} \gamma^{k} \mathbb{E}(x_k^\top S x_k + u_k^\top R u_k) + \gamma^{N} \mathbb{E}(x_N^\top S x_N)$$
...
The average cost objective is studied a lot more than the discounted cost objective in the control theory literature, which is the oppositive of the RL literature.

Although the average cost setting seems to make sense especially for finite sample analysis,
we only cover the discounted setting in the rest part of this post to align with the RL algorithms.
...
Since we know that the action value funciton is quadratic, we can build the quadratic basis functions from the state-action pair $$[x\ u]$$ and approximate the action value as a linear combination of them:
 ...
 where $$\theta$$ is the weight vector and the $$ \phi(x, u) $$ is the quadratic basis feature vector: $$ \phi(v) = [v_1^2, \cdots, v_1v_n, v_2^2, \cdots, v_2v_n, v_{n-1}^2, v_{n-1}v_n, \cdots v_n^2] $$ 
 
 There are $$(n+m)^2$$ entries in the matrix $$H$$ but we do not need to estimate that many parameters due to $$H$$ being symmetric. 

 The dimensionality of $$\theta$$ and $$ \phi(x, u) $$ is `(n+m+1)(n+m)/2`. $$\theta$$ can be viewed as a flatten version of the upper triangular of matrix $$H$$. 
...
$$ J(u) = \int_0^T \gamma^{t} \mathbb{E}[c_t] dt = \int_0^T \gamma^{t} \mathbb{E}[x(t)^\top S x(t) + u(t)^\top R u(t)] dt +  \gamma^{T}\mathbb{E}[x(T)^\top S x(T)]$$

**Average Cost**:

$$ J(u) = \lim_{T\to \infty} \frac{1}{T} \int_0^T \mathbb{E}[c_t] dt =\lim_{T\to \infty} \frac{1}{T} \int_0^T \mathbb{E}[x(t)^\top S x(t) + u(t)^\top R u(t)] dt $$
...
For `stability`, we need to make sure the eigenvalues of $$A+BK$$ are all negative.
...
- The system dynamic will be approximated as piece-wise linear, which is another layer of approximation on top of the time-invariant system discussed in this post.

## Show me the code

Well, sorry that the code is not fully developed at this point to share.

But Stephen Tu has an awesome tutorial on running LQR on Cartpole Swing-Up:
