...
where the $$P_*$$ is the solution to Algebraic Riccati Equation (ARE):

$$ P_* = S + \gamma A^\top P_* A - \gamma^2 A^\top P_* B(R+\gamma B^\top P_* B)^{-1} B^\top P_* A $$

$$P_*$$ is `n x n` symmetric matrix. 
...
The value function $$V_\pi(x_k)$$ can be expressed as $$V_\pi(x_k) = x_k^\top P_\pi x_k$$
where $$P_\pi$$ is a cost matrix associated with the policy $$\pi: \mathcal{X} \to \mathcal{U}$$ that maps the states to actions. 
...
$$\begin{align}Q_\pi(x, u) &= c(x,u) + \gamma  V_\pi( Ax + Bu) \\ &= [x\ u]^\top \begin{bmatrix} S + \gamma A^\top P_\pi A &  \gamma A^\top P_\pi B \\ \gamma B^\top P_\pi A &  R+\gamma B^\top P_\pi B \end{bmatrix} [x\ u] \\&= [x\ u]^\top \begin{bmatrix} H_{11} &  H_{12} \\ H_{21} &  H_{22} \end{bmatrix} [x\ u] \end{align}$$
